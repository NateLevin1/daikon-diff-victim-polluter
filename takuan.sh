...
SHOULD_USE_GDATE=$(command -v gdate > /dev/null);
getCurrentMillis() {
    if $SHOULD_USE_GDATE; then
        echo $(gdate +%s%3N)
    else
        echo $(date +%s%3N)
    fi
...
POST_SETUP_START_TIME=$(getCurrentMillis)
if [[ -z "${NO_GEN}" ]]; then
...
if [[ -z "${NO_FIND_CLEANER}" ]]; then
    CLEANER_FINDER_START_TIME=$(getCurrentMillis)
    ...
    echo "[!] [Cleaner-Finder] Took $(( $(getCurrentMillis) - $CLEANER_FINDER_START_TIME ))ms"
fi
